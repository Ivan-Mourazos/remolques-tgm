...
export default function ParametrosPage() {
  ...
  if (!p) {
    return (
      <main className="text-sm text-muted-2">
  ...
  return (
    <main className="max-w-4xl">
            ...
            <tr key={r.nombre} className="border-b border-line">
              ...
              {(["delante", "atras", "lateralSoloAtras", "lateralSoloDelante"] as const).map((campo) => (
                <td key={campo} className="p-1">
                  <input type="number" step="0.5" className="w-20 rounded-lg border border-line bg-white px-2 py-1 tabular-nums outline-none focus:border-gold"
      ...
      <button onClick={guardar} className="rounded-lg bg-deep px-4 py-2 text-sm font-bold text-white shadow-[0_7px_20px_rgb(9_39_44/0.20)] transition hover:bg-deep-2">
        Guardar parámetros
      </button>
      {aviso && (
        <p className={`mt-2 text-xs font-semibold ${aviso.error ? "text-red-700" : "text-muted"}`}>
          {aviso.texto}
        </p>
      )}
      </section>
